        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Số lượng thẻ sở hữu: {cards?.length}
            </p>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {cards.map((card) => (
              <Link
                href={`/cards/${card._id}`}
                key={card._id}
                className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100 group cursor-pointer block"
              >
                <div
                  key={card._id}
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="font-bold text-medium text-gray-900 mb-3 line-clamp-1">
                      {card.name}
                    </h3>
                    <div className="flex justify-between items-center border-t border-gray-100 pt-4 mt-2">
                      <div>
                        ...
                        <p className="text-red-500 font-bold">
                          {card.annualFee.toLocaleString("vi-VN")} ₫
                        </p>
                      </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 md:col-span-1">
                    ...
                    <select
                      required
                      className="w-full p-2.5 bg-white text-gray-900 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                      value={formData.bank}
                      onChange={(e) =>
                    ...
                    >
                      ...
                      {banks.map((b) => (
                        <option key={b._id} value={b.shortname}>
                          {b.shortname}
                        </option>
                      ))}
                    </select>
                ...
                <div>
                  ...
                  <input
                    required
                    placeholder="VD: StepUP Cashback"
                    className="w-full p-2.5 bg-white text-gray-900 placeholder-gray-400 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition-all"
                    value={formData.name}
                    onChange={(e) =>
                ...
                <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-100">
                  <button
                    type="button"
                    onClick={closeModal}
                    className="px-5 py-2.5 text-gray-900 font-medium hover:bg-gray-100 rounded-lg transition-colors"
                  >
                    Hủy bỏ
                  </button>
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-medium transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2"
                  >
                    {isSubmitting
                      ? "Đang xử lý..."
                      : editingId
